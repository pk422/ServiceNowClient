...
namespace ServiceNow.AttachmentAPI
{
	public class AttachmentAPIClient<T>:IAttachmentAPIClient<T> where T : Record
	{
		private String _TableName;
		private String _InstanceName;
		private String _FieldList;
		private WebClient ServiceNowClient;
		public AttachmentAPIClient(String tableName, String instanceName, NetworkCredential credentials)
		{
		...
		public RESTSingleResponse<T> Post(T record)
		{
			...
			try
			{
				...
				string data = JsonConvert.SerializeObject(record, new ResourceLinkConverter());
				response.RawJSON = ServiceNowClient.UploadString(URL + "?&sysparm_fields=sys_id,name,description", "POST", data);
